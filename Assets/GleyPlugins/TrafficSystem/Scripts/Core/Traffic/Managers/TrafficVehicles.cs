...
namespace GleyTrafficSystem
{
    ...
    public class TrafficVehicles : MonoBehaviour
    {
        ...
        public void RemoveCar (VehicleComponent car, GameObject carObject)
        {
            VehicleComponent newCar = Instantiate(carObject, Vector3.zero, Quaternion.identity, trafficHolder).GetComponent<VehicleComponent>();
            newCar.name = car.name;
            newCar.SetIndex(car.GetIndex());
            for (int i =0; i< allVehicles.Count; i++)
            {
                if (allVehicles[i].name == carObject.name)
                {
                    allVehicles[i] = newCar;
                    Debug.Log("swap");
                    DisableVehicle(newCar.GetIndex()) ; 
                    break;
                }
            }
        ...
        public void RemoveCar(int num)
        {
            ...
            /* VehicleComponent newCar = Instantiate(carObject, Vector3.zero, Quaternion.identity).GetComponent<VehicleComponent>();
