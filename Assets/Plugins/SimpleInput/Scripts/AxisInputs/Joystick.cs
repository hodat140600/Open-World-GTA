...
#pragma warning restore 0649
            ...
            else
            {
                if (direction.sqrMagnitude > movementAreaRadiusSqr)
                {
                    ...
                    if (canFollowPointer)
                    {
                        ...
                        joystickTR.anchoredPosition = joystickTR.anchoredPosition
                            .Clamp(
                                dynamicJoystickMovementArea.rect.min,
                                dynamicJoystickMovementArea.rect.max);

                        // joystickTR.localPosition =  (joystickTR.localPosition.normalized * MovementRange); // + (Vector2)joystickInitialPos;
                    }
        ...
        public float MovementRange = 300f;
        ...
        private void OnUpdate()
        {
            ...
            if (joystickHeld)
                opacity = Mathf.Min(.75f, opacity + Time.unscaledDeltaTime * 4f);
            else
                opacity = Mathf.Max(.25f, opacity - Time.unscaledDeltaTime * 1f);

            if (_resetPosition && Math.Abs(opacity - .3f) < .02f)
            {
