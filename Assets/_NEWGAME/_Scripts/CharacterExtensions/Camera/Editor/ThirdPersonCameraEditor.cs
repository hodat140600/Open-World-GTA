...
{
    [CustomEditor(typeof(ThirdPersonCamera), true)]
    ...
    public class ThirdPersonCameraEditor : Editor
    {
        ...
        void OnEnable()
        {
            m_Logo = (Texture2D)Resources.Load("ThirdPersonExtensions/tp_camera", typeof(Texture2D));
            indexSelected = 0;
            tpCamera = (ThirdPersonCamera)target;
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.BeginVertical("Third Person Camera by DatHQ", "window");
            ...
            tpCamera.CameraStateList = (ThirdPersonCameraListData)EditorGUILayout.ObjectField("CameraState List", tpCamera.CameraStateList, typeof(ThirdPersonCameraListData), false);
            ...
            if (GUILayout.Button(new GUIContent("New CameraState")))
            {
                if (tpCamera.CameraStateList.tpCameraStates == null)
                    tpCamera.CameraStateList.tpCameraStates = new List<ThirdPersonCameraState>();

                tpCamera.CameraStateList.tpCameraStates.Add(new ThirdPersonCameraState("New State" + tpCamera.CameraStateList.tpCameraStates.Count));
        ...
        void StateData(ThirdPersonCameraState camState)
        {
            EditorGUILayout.Space();
        ...
        void DrawLookPoint(ThirdPersonCameraState camState)
        {
        ...
        void FreeDirectionalMode(ThirdPersonCameraState camState)
        {
        ...
        void FixedAngleMode(ThirdPersonCameraState camState)
        {
            DrawFloatField("Distance", ref camState.defaultDistance);
            DrawToogleField("Use Zoom", ref camState.useZoom);
            if (camState.useZoom)
        ...
        void FixedPointMode(ThirdPersonCameraState camState)
        {
            DrawFloatField("Smooth Follow", ref camState.smooth);
            DrawSliderField("Field of View", ref camState.fov, 1, 179);
        ...
        {
            foreach (ThirdPersonCameraState state in tpCamera.CameraStateList.tpCameraStates)
        ...
        [MenuItem("DatHQ/Basic Locomotion/Resources/New CameraState List Data")]
        static void NewCameraStateData()
        {
            ScriptableObjectUtility.CreateAsset<ThirdPersonCameraListData>();
        }

        private string[] getListName(List<ThirdPersonCameraState> list)
        {
