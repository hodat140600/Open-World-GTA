...
namespace _GAME._Scripts._CharacterController._Actions
{
    ...
    [ClassHeader("GENERIC ACTION", "Use the vTriggerGenericAction to trigger a simple animation.\n<b><size=12>You can use <color=red>vGenericActionReceiver</color> component to filter events by action name</size></b>", iconName = "ThirdPersonExtensions/triggerIcon")]
    public class GenericAction : ActionListener
    {
        ...
        [LeoReadOnly] public TriggerGenericAction triggerAction;
        [LeoReadOnly, SerializeField]
        ...
        public OnActionHandle OnEnterTriggerAction;
        public OnActionHandle OnExitTriggerAction;
        public OnActionHandle OnStartAction;
        public OnActionHandle OnCancelAction;
        public OnActionHandle OnEndAction;

        internal Camera mainCamera;
        internal ThirdPersonInput tpInput;
        ...
        {
            internal TriggerGenericAction action;
            internal bool isValid;
            internal ActionStorage()
            {

            }
            internal ActionStorage(TriggerGenericAction action)
            {
                this.action = action;
                action.OnValidate.AddListener((o) => { isValid = true; });
                action.OnInvalidate.AddListener((o) => { isValid = false; });
            }
            public static implicit operator TriggerGenericAction(ActionStorage storage)
            {
                return storage.action;
            }
            public static implicit operator ActionStorage(TriggerGenericAction action)
            {
        ...
        protected virtual void CheckForTriggerAction()
        {
            ...
            TriggerGenericAction _triggerAction = GetNearAction();
        ...
        protected TriggerGenericAction GetNearAction()
        {
            ...
            TriggerGenericAction _targetAction = null;
            ...
            foreach (var key in actions.Keys)
            {
                if (key)
                {
                    try
                    {
                        TriggerGenericAction action = actions[key];
        ...
        protected virtual void AnimationBehaviour()
        {
            ...
            if (playingAnimation)
            {
                ...
                if (actionStarted && !triggerAction.endActionManualy && (triggerAction.inputType != TriggerGenericAction.InputType.GetButtonTimer || !triggerAction.playAnimationWhileHoldingButton) && tpInput.cc.animatorStateInfos.GetCurrentNormalizedTime(triggerAction.animatorLayer) >= triggerAction.endExitTimeAnimation)
                {
                    if (debugMode)
                    {
            ...
            else if (doingAction && actionStarted && (triggerAction == null || !triggerAction.endActionManualy))
            {
                ...
                if (triggerAction != null && triggerAction.inputType == TriggerGenericAction.InputType.GetButtonTimer && triggerAction.playAnimationWhileHoldingButton)
                {
                    return;
                }
        ...
        public override void OnActionEnter(Collider other)
        {
            ...
            if (other != null && other.gameObject.CompareTag(actionTag))
            {
                ...
                {
                    TriggerGenericAction _triggerAction = other.GetComponent<TriggerGenericAction>();
        ...
        public override void OnActionExit(Collider other)
        {
            ...
            {
                TriggerGenericAction action = actions[other];
        ...
        /// <summary>
        /// End Action Manualy if <see cref="TriggerGenericAction.endActionManualy"/> equals true
        /// </summary>
        ...
        public virtual void TriggerActionInput()
        {
            ...
            // AutoAction
            if (triggerAction.inputType == TriggerGenericAction.InputType.AutoAction && actionConditions)
            {
                TriggerActionEvents();
                TriggerAnimation();
            }
            // GetButtonDown
            else if (triggerAction.inputType == TriggerGenericAction.InputType.GetButtonDown && actionConditions)
            {
            ...
            // GetDoubleButton
            else if (triggerAction.inputType == TriggerGenericAction.InputType.GetDoubleButton && actionConditions)
            {
            ...
            else if (triggerAction.inputType == TriggerGenericAction.InputType.GetButtonTimer)
            {
        ...
        public virtual IEnumerator DestroyActionDelay(TriggerGenericAction triggerAction)
        {
