...
namespace _GAME._Scripts._CharacterController
{
    ...
    public class ThirdPersonAnimator : ThirdPersonMotor
    {
        ...
        public virtual void UpdateAnimatorParameters()
        {
            ...
            animator.SetBool(AnimatorParameters.IsSliding, isSliding && !isRolling);
            ...
            if (!isGrounded)
            {
                animator.SetFloat(AnimatorParameters.VerticalVelocity, verticalVelocity);
            }
            ...
            //if (!lockAnimMovement)
            {
                if (isStrafing)
                {
                    animator.SetFloat(AnimatorParameters.InputHorizontal, horizontalSpeed, strafeSpeed.animationSmooth, Time.fixedDeltaTime);
                    animator.SetFloat(AnimatorParameters.InputVertical, verticalSpeed, strafeSpeed.animationSmooth, Time.fixedDeltaTime);
                }
                else
                {
                    animator.SetFloat(AnimatorParameters.InputVertical, verticalSpeed, freeSpeed.animationSmooth, Time.fixedDeltaTime);
                    animator.SetFloat(AnimatorParameters.InputHorizontal, 0, freeSpeed.animationSmooth, Time.fixedDeltaTime);
                }

                animator.SetFloat(AnimatorParameters.InputMagnitude, Mathf.LerpUnclamped(inputMagnitude, 0f, stopMoveWeight), isStrafing ? strafeSpeed.animationSmooth : freeSpeed.animationSmooth, Time.fixedDeltaTime);

                if (useLeanMovementAnim && inputMagnitude >= 0.1f)
                {
                    animator.SetFloat(AnimatorParameters.RotationMagnitude, rotationMagnitude, leanSmooth, Time.fixedDeltaTime);
                }
                else if (useTurnOnSpotAnim && inputMagnitude < 0.1f)
                {
                    animator.SetFloat(AnimatorParameters.RotationMagnitude, (float)System.Math.Round(rotationMagnitude, 2), rotationMagnitude == 0 ? 0.1f : 0.01f, Time.fixedDeltaTime);
                }
            }
        }
