...
namespace _GAME._Scripts._CharacterController
{
    ...
    public class ThirdPersonMotor : Character, IAnimatorStateInfoController
    {
        ...
        [EditorToolbar("Locomotion", order = 0)]
        ...
        [HelpBox("When 'Use RootMotion' is checked, make sure to reset all speeds to zero to use the original root motion velocity.")]
        ...
        [Separator("Extra Movement Settings")]
        ...
        [HelpBox("Set the FixedTimeStep to match the FPS of your Game, \nEx: If your game aims to run at 30fps, select FPS30 to match the FixedUpdate Physics")]
        public CustomFixedTimeStep customFixedTimeStep = CustomFixedTimeStep.FPS60;

        [EditorToolbar("Jump / Airborne", order = 3)]

        [HelpBox("Jump only works via Rigidbody Physics, if you want Jump that use only RootMotion make sure to use the AnimatorTag 'CustomAction' ")]
        ...
        [_Scripts.LeoHideInInspector("Roll use gravity inflence")]
        public bool rollUseGravity = true;
        [_Scripts.LeoHideInInspector("rollUseGravity")]
        ...
        [EditorToolbar("Grounded", order = 3)]
        ...
        [HelpBox("Character will stop moving, ex: walls - set the layer to nothing to not use")]
        ...
        internal float defaultSpeedMultiplier = 1.5f;
        ...
        protected override void TriggerDamageReaction(Damage damage)
        {
            if (!customAction)
            {
        ...
        public virtual void MoveCharacter(Vector3 _direction)
        {
            ...
            inputSmooth = Vector3.Lerp(inputSmooth, input, (isStrafing ? strafeSpeed.movementSmooth : freeSpeed.movementSmooth) * (useRootMotion ? dTime.deltaTime : dTime.fixedDeltaTime));
            ...
            Vector3 targetPosition = (useRootMotion ? animator.rootPosition : _rigidbody.position) + _direction * (moveSpeed * speedMultiplier) * (useRootMotion ? dTime.deltaTime : dTime.fixedDeltaTime);
            Vector3 targetVelocity = (targetPosition - transform.position) / (useRootMotion ? dTime.deltaTime : dTime.fixedDeltaTime);
        ...
        protected virtual void CheckFallDamage()
        {
            ...
            if (fallHeight > 0)
            {
                ...
                TakeDamage(new Damage(damage, true));
            }
        }
        ...
        {
            [HelpBox("Higher means faster/responsive movement, lower means smooth movement")]
            [Range(1f, 20f)]
            public float movementSmooth = 6f;
            [HelpBox("Lower means faster transitions between animations, higher means slower")]
            [Range(0f, 1f)]
