...
namespace _GAME._Scripts
{
    ...
    [CustomEditor(typeof(FootStep), true)]
    public class FootStepEditor : Editor
    {
        ...
        void OnEnable()
        {
            m_Logo = (Texture2D)Resources.Load("ThirdPersonExtensions/footStepIcon", typeof(Texture2D));
            CheckColliders();
        }
        ...
        bool CheckColliders()
        {
            ...
            var transform = (serializedObject.targetObject as FootStep).transform;
            if (transform == null) return false;
            var animator = transform.GetComponent<Animator>();
            if (animator == null) return false;
            var leftFoot = animator.GetBoneTransform(HumanBodyBones.LeftFoot);
            FootStepTrigger leftFoot_trigger = null;
            if (leftFoot != null)
                leftFoot_trigger = leftFoot.GetComponentInChildren<FootStepTrigger>();
            ...
            FootStepTrigger rightFoot_trigger = null;
            if (rightFoot != null)
                rightFoot_trigger = rightFoot.GetComponentInChildren<FootStepTrigger>();
        ...
        void DrawFootStepList()
        {
            ...
            if (footStepList != null)
            {
                ...
                if (GUILayout.Button("New", EditorStyles.miniButton))
                {
                    ...
                    var go = new GameObject("Trigger-" + footStepList.arraySize.ToString("00"), typeof(FootStepTrigger), typeof(SphereCollider));
                    go.GetComponent<SphereCollider>().radius = 0.05f;
                    go.transform.position = (target as FootStep).transform.position;
                    go.layer = LayerMask.NameToLayer("Ignore Raycast");
                    go.transform.parent = (target as FootStep).transform;

                    footStepList.GetArrayElementAtIndex(footStepList.arraySize - 1).objectReferenceValue = go.GetComponent<FootStepTrigger>();
                }
                GUILayout.EndHorizontal();
        ...
        bool DrawFootStepElement(SerializedProperty list, SerializedProperty footStepElement, int index)
        {
            ...
            {
                if (footStepElement.objectReferenceValue as FootStepTrigger != null)
                {
                    DestroyImmediate((footStepElement.objectReferenceValue as FootStepTrigger).gameObject);
