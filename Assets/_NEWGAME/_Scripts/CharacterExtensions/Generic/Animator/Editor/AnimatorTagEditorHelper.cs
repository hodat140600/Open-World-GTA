...
{
    [InitializeOnLoad]
    public static class AnimatorTagEditorHelper
    {
        private static GUIStyle _customTagStyle;

        private static GUIStyle _defaultTagStyle;

        static AnimatorTagEditorHelper()
        {
            TAGS = new Dictionary<string, TooltipAttribute>();
            FieldInfo[] fields = typeof(AnimatorTags).GetFields(BindingFlags.Public | BindingFlags.Static);
