...
{
    [ClassHeader("Body Snapping Control", openClose = false)]
    public class BodySnappingControl : ExtendMonoBehaviour
    {
        [LeoButton("Create New BodyStruct", "NewBodyStruct", typeof(BodySnappingControl), false)]
        [LeoButton("Load Bones", "LoadBones", typeof(BodySnappingControl), false)]
        //[vHelpBox("To create a new BodyStruct for a new Character to go: \n<b>*Menu Create/DatHQ/New Body Struct</b>, you can set for both Humanoid or Generic Rig")]
        public BodyStruct bodyStruct;
        public bool showLabels;
        [@HideInInspector] public List<BoneTransformSnapping> boneSnappingList = new List<BoneTransformSnapping>();
        ...
        {
            BodyStruct newBodyStruct = ScriptableObject.CreateInstance<BodyStruct>();
        ...
        public virtual void LoadBones()
        {
            ...
            var bones = bodyStruct ? bodyStruct.bones : BodyStruct.GetHumanBones();
            if (bodyStruct)
            {
            ...
            if (bones.Count > 0)
            {
                for (int i = 0; i < bones.Count; i++)
                {
                    ...
                    BoneTransformSnapping b = boneSnappingList.Find(_b => _b.name.Equals(bones[i].name));
                    if (b == null)
                    {
                        b = new BoneTransformSnapping();
                        b.name = bones[i].name;
                        b.bone = bone;
        ...
        protected virtual Transform GetBoneByName(string name)
        {
            ...
            List<Transform> childrens = root.gameObject.GetComponentsInChildren<Transform>(true).dToList();
            Transform t = null;
        ...
        [System.Serializable]
        public class BoneTransformSnapping
        {
            public string name;
