...
namespace _GAME._Scripts._ItemManager
{
    ...
    [CustomEditor(typeof(AmmoListData), true)]
    public class AmmoListDataEditor : Editor
    {
        GUISkin skin;
        ...
        void LoadAmmos()
        {
            if (listData.itemListDatas.Count > 0)
            {
                ...
                {
                    var ammoItems = listData.itemListDatas[i].items.FindAll(item => item.type == _GAME._Scripts._ItemManager.ItemType.Ammo);
                    for (int a = 0; a < ammoItems.Count; a++)
                    {
                        if (!listData.ammos.Exists(ammo => ammo.ammoID == ammoItems[a].id))
                        {
                            listData.ammos.Add(new Ammo(ammoItems[a].name, ammoItems[a].id));
                        }
                    }
        ...
        void AddAmmo()
        {
            ...
            {
                listData.ammos.Add(new Ammo(ammoName, id, amount));
                inAddAmmo = false;
            }
            GUILayout.EndHorizontal();
            GUILayout.EndVertical();
        ...
        [MenuItem("DatHQ/Shooter/Create new AmmoListData")]
        public static void CreateAmmoList()
        {
            ScriptableObjectUtility.CreateAsset<AmmoListData>();
        }
    }
}
