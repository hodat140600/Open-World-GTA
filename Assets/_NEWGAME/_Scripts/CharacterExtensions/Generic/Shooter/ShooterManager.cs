...
namespace _GAME._Scripts._CharacterController._Shooter
{
    ...
    [ClassHeader("SHOOTER MANAGER", iconName = "shooterIcon")]
    public class ShooterManager : ExtendMonoBehaviour, IWeaponEquipmentListener
    {
        #region variables

        [System.Serializable]
        public class OnReloadWeapon : UnityEvent<ShooterWeapon> { }
        public class OnShootWeapon : UnityEvent<ShooterWeapon> { }
        public delegate void AmmoHandle(int ammoID, ref int ammo);
        [EditorToolbar("Melee Overrides")]
        [HelpBox("Behaviour when Shooter Weapon is Disabled (equipped but disabled)")]
        public bool canUseMeleeBlock_H = true;
        public bool canUseMeleeWeakAttack_H = true;
        public bool canUseMeleeStrongAttack_H = true;
        [HelpBox("Behaviour when Shooter Weapon is Enabled (equipped and enabled)")]
        ...
        public TagMask ignoreTags = new TagMask("Player");
        [Tooltip("Layer to block aim")]
        public LayerMask blockAimLayer = 1 << 0;

        [EditorToolbar("Cancel Reload")]
        [HelpBox("You can call the <b>CancelReload</b> method using events to interupt the reload routine and animation, for example, when doing an Custom Action or receiving a specific hitReaction ID")]
        ...
        [Separator("--- Start PlayMode to edit the IK Adjust ---")]
        public WeaponIKAdjustList weaponIKAdjustList;
        ...
        [EditorToolbar("LockOn")]
        [Separator("LockOn (need the shooter lockon component)")]
        ...
        [EditorToolbar("HipFire")]
        [Separator("HipFire Options")]
        ...
        [EditorToolbar("Camera Sway")]
        [Separator("Camera Sway Settings")]
        ...
        [EditorToolbar("Weapons")]
        public ShooterWeapon rWeapon, lWeapon;
        ...
        public OnShootWeapon onOutAmmoWeapon;
        [@HideInInspector]
        public AmmoDisplay ammoDisplayR, ammoDisplayL;
        [@HideInInspector]
        public _Camera.ThirdPersonCamera tpCamera;
        [@HideInInspector]
        ...
        protected WeaponIKAdjust currentWeaponIKAdjust;
        /// <summary>
        ...
        public _Melee.OnEquipWeaponEvent onEquipWeapon;
        ...
        public virtual void Start()
        {
            ...
            if (ammoManager != null)
            {
                ammoManager.updateTotalAmmo = new AmmoManager.OnUpdateTotalAmmo(AmmoManagerWasUpdated);
            }

            var tpInput = GetComponent<ThirdPersonController>();
            ...
            if (animator)
            {
                ...
                var weaponR = _rightHand.GetComponentInChildren<ShooterWeapon>();
                var weaponL = _lefttHand.GetComponentInChildren<ShooterWeapon>();
                if (weaponR != null)
                {
        ...
        public virtual void SetLeftWeapon(GameObject weapon)
        {
            if (weapon != null)
            {
                var w = weapon.GetComponent<ShooterWeapon>();
                SetLeftWeapon(w);
            }
            else
            {
        ...
        protected virtual void SetLeftWeapon(ShooterWeapon weapon)
        {
            lWeapon = weapon;
            if (lWeapon)
            {
        ...
        protected virtual void SetRightWeapon(ShooterWeapon weapon)
        {
            rWeapon = weapon;
            if (rWeapon)
            {
        ...
        protected virtual void CollectExtraAmmo(ShooterWeapon weapon)
        {
        ...
        protected virtual void HideAmmoDisplay(AmmoDisplay ammoDisplay)
        {
        ...
        {
            var ammoDisplays = FindObjectsOfType<AmmoDisplay>();
            if (ammoDisplays.Length > 0)
            {
                if (!ammoDisplayL)
                {
                    ammoDisplayL = ammoDisplays.dToList().Find(d => d.displayID == leftWeaponAmmoDisplayID);
                }

                if (!ammoDisplayR)
                {
                    ammoDisplayR = ammoDisplays.dToList().Find(d => d.displayID == rightWeaponAmmoDisplayID);
                }
            }
        }
        ...
        protected virtual IEnumerator AddAmmoToWeapon(ShooterWeapon weapon, float delayTime)
        {
            isReloading = true;
        ...
        public virtual void CancelReload(Damage damage)
        {
        ...
        public virtual void LoadAllAmmo(ShooterWeapon weapon)
        {
            if (!weapon)
            {
                return;
        ...
        public virtual Ammo WeaponAmmo(ShooterWeapon weapon)
        {
            if (!weapon)
            {
                return null;
        ...
        public virtual ShooterWeapon CurrentWeapon
        {
            get
            {
                var _weapon = rWeapon ?
        ...
        public virtual void SetIKAdjustList(WeaponIKAdjustList weaponIKAdjustList)
        {
        ...
        protected virtual void UpdateTotalAmmo(ShooterWeapon weapon, ref int targetTotalAmmo, int displayId)
        {
            if (!weapon)
            {
                return;
        ...
        protected virtual Vector3 HipFirePrecision(Vector3 _aimPosition)
        {
            ...
            hipfirePrecisionAngle = Random.Range(-1000, 1000);
