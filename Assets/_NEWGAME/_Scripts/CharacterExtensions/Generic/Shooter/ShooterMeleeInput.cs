...
namespace _GAME._Scripts._CharacterController
{
    ...
    using _IK;
    [ClassHeader("SHOOTER/MELEE INPUT", iconName = "inputIcon")]
    public class ShooterMeleeInput : MeleeCombatInput, IShooterIKController, PlayerController.ILockCamera
    {
        ...
        [@HideInInspector] public ShooterManager shooterManager;
        ...
        protected HeadTrack headTrack;

        protected bool lastRotateWithCamera;
        protected ControlAimCanvas _controlAimCanvas;
        ...
        public virtual IKSolver LeftIK { get; set; }

        public virtual IKSolver RightIK { get; set; }

        public virtual WeaponIKAdjustList WeaponIKAdjustList
        {
            get
            {
                if (shooterManager)
        ...
        public virtual WeaponIKAdjust CurrentWeaponIK
        {
            get
            {
                if (shooterManager)
        ...
        public virtual ControlAimCanvas controlAimCanvas
        {
            get
            {
                if (!_controlAimCanvas)
                {
                    _controlAimCanvas = FindObjectOfType<ControlAimCanvas>();
                    if (_controlAimCanvas)
                    {
        ...
        /// </summary>
        public virtual ShooterWeapon CurrentActiveWeapon
        {
            get
            {
        ...
        public virtual void AimInput()
        {
            ...
            if (cc.locomotionType == ThirdPersonMotor.LocomotionType.OnlyFree)
            {
            ...
            if (cc.locomotionType == ThirdPersonMotor.LocomotionType.FreeWithStrafe)
            {
                if (!cc.lockInStrafe)
                {
        ...
        public virtual void HandleShotCount(ShooterWeapon weapon, bool weaponInput = true)
        {
            ...
            if (weapon.chargeWeapon)
            {
                ...
                animator.SetFloat(AnimatorParameters.PowerCharger, weapon.powerCharge);
            }
        ...
        public virtual void DoShots()
        {
            if (CanDoShots())
            {
                animator.SetFloat(AnimatorParameters.Shot_ID, shooterManager.GetShotID());
        ...
        /// <summary>
        /// Change side view of the <seealso cref="_Camera.ThirdPersonCamera"/>
        /// </summary>
        ...
        protected override void UpdateMeleeAnimations()
        {
            ...
            if ((shooterManager == null || !CurrentActiveWeapon) && meleeManager)
            {
                ...
                onlyArmsLayerWeight = Mathf.Lerp(onlyArmsLayerWeight, 0, 6f * dTime.deltaTime);
        ...
        {
            cc.animator.SetFloat(AnimatorParameters.MoveSet_ID, defaultMoveSetID, .2f, Time.deltaTime);
        }
        ...
        public virtual void ResetShooterAnimations()
        {
            ...
            // set the uppbody id (armsonly layer)
            animator.SetFloat(AnimatorParameters.UpperBody_ID, 0, .2f, dTime.deltaTime);
            // set if the character can aim or not (upperbody layer)
            animator.SetBool(AnimatorParameters.CanAim, false);
            // character is aiming
            animator.SetBool(AnimatorParameters.IsAiming, false);
            // animator.SetBool(vAnimatorParameters.IsHipFire, false);
            // turn on the onlyarms layer to aim 
        ...
        protected virtual void UpdateShooterAnimations()
        {
            ...
            // turn on the onlyarms layer to aim 
            onlyArmsLayerWeight = Mathf.Lerp(onlyArmsLayerWeight, CurrentActiveWeapon || isEquipping ? 1f : 0f, shooterManager.onlyArmsSpeed * dTime.deltaTime);
            animator.SetLayerWeight(onlyArmsLayer, onlyArmsLayerWeight);
            ...
            if (CurrentActiveWeapon && !shooterManager.useDefaultMovesetWhenNotAiming || IsAiming || isReloading)
            {
                ...
                animator.SetFloat(AnimatorParameters.MoveSet_ID, shooterMoveSetID, .1f, dTime.deltaTime);
            }
            else if (!CurrentActiveWeapon && !shooterManager.useDefaultMovesetWhenNotAiming || shooterManager.useDefaultMovesetWhenNotAiming)
            {
                // set the move set id (base layer) 
                animator.SetFloat(AnimatorParameters.MoveSet_ID, defaultMoveSetID, .1f, dTime.deltaTime);
            }

            // set the isBlocking false while using shooter weapons
            animator.SetBool(AnimatorParameters.IsBlocking, isBlocking);
            // set the uppbody id (armsonly layer)
            animator.SetFloat(AnimatorParameters.UpperBody_ID, shooterManager.GetUpperBodyID());
            // set if the character can aim or not (upperbody layer)
            animator.SetBool(AnimatorParameters.CanAim, aimConditions);
            // character is aiming
            animator.SetBool(AnimatorParameters.IsAiming, IsAiming);
            ...
            AnimatorParameter ap = new AnimatorParameter(animator, "IsReloading");
        ...
        public override void UpdateCameraStates()
        {
            ...
            if (tpCamera == null)
            {
                tpCamera = FindObjectOfType<_Camera.ThirdPersonCamera>();
                if (tpCamera == null)
                {
                    return;
                }
        ...
        protected virtual void UpdateIKAdjust(bool isUsingLeftHand)
        {
            ...
            {
                LeftIK = new IKSolver(animator, AvatarIKGoal.LeftHand);
                LeftIK.UpdateIK();
            }
            if (RightIK == null || !RightIK.isValidBones)
            {
                RightIK = new IKSolver(animator, AvatarIKGoal.RightHand);
                RightIK.UpdateIK();
            }
            ...
            weaponIKWeight = Mathf.Lerp(weaponIKWeight, isValidIK ? 1 : 0, 25f * dTime.deltaTime);
            if (weaponIKWeight <= 0)
            {
                return;
            }
        ...
        protected virtual void ApplyOffsets(IKSolver weaponHand, IKSolver supportHand, bool isValidIK = true)
        {
        ...
        protected virtual void ApplyOffsetToTargetBone(IKOffsetTransform iKOffset, Transform target, bool isValidIK)
        {
            try
            {
                target.localPosition = Vector3.Lerp(target.localPosition, isValidIK ? iKOffset.position : Vector3.zero, shooterManager.ikAdjustSmooth * dTime.deltaTime);
                target.localRotation = Quaternion.Lerp(target.localRotation, isValidIK ? Quaternion.Euler(iKOffset.eulerAngles) : Quaternion.Euler(Vector3.zero), shooterManager.ikAdjustSmooth * dTime.deltaTime);
            }
            catch
            {
        ...
        protected virtual void UpdateArmsIK(bool isUsingLeftHand = false)
        {
            ...
            if (targetIK != null)
            {
                ...
                {
                    supportIKWeight = Mathf.Lerp(supportIKWeight, 1, shooterManager.armIKSmoothIn * dTime.deltaTime);
                }
                else
                {
                    supportIKWeight = Mathf.Lerp(supportIKWeight, 0, shooterManager.armIKSmoothOut * dTime.deltaTime);
                }
