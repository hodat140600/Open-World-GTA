...
namespace _GAME._Scripts._CharacterController
{
    ...
    [ClassHeader("THROW MANAGER")]
    public class ThrowManager : ExtendMonoBehaviour
    {
        #region variables
        ...
        public bool debug;
        [Separator("Only for ThirdPerson Camera Style")]
        [Tooltip("The Third person camera right will be applied as offset to throw start point")]
        public bool useCameraRightAsOffset;
        [Tooltip("Increase or decrease the Offset right value"), _Scripts.LeoHideInInspector("useCameraRightAsOffset")]
        ...
        public virtual ThrowUI ui
        {
            get
            {
                if (!_ui)
                {
                    _ui = FindObjectOfType<ThrowUI>();
                    if (_ui)
                    {
        ...
        protected virtual IEnumerator Start()
        {
            ...
            tpInput = GetComponentInParent<ThirdPersonInput>();
        ...
        protected virtual void DrawTrajectory()
        {
            ...
            if (throwEnd)
            {
                ...
                if (objectToThrow.gameObject != lastThrowable)
                {
                    ...
                    explosive = lastThrowable.GetComponentInChildren<Explosive>();
        ...
        protected virtual Vector3 topdownAimPoint
        {
            get
            {
                var pos = MousePositionHandler.Instance.WorldMousePosition(obstacles);
        ...
        protected virtual Vector3 sideScrollAimPoint
        {
            get
            {
                var localPos = transform.InverseTransformPoint(MousePositionHandler.Instance.WorldMousePosition(obstacles));
                localPos.x = 0;
