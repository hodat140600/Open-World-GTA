...
{
    [ClassHeader("Message Sender", "Requires a MessageReceiver to receive messages.", openClose = false)]
    public class MessageSender : ExtendMonoBehaviour
    {
        [System.Serializable]
        public class Message
        {
            ...
            [HelpBox("- sendByTrigger (You can use SimpleTrigger to verify the Player and Send messages using Events by calling the 'OnTrigger' method")]
            public bool sendByTrigger;
            public List<MessageReceiver> defaultReceivers;
        }
        [System.Serializable]
        ...
        {
            Message _message = messages.Count > 0 && messageIndex < messages.Count ? messages[messageIndex] : null;
            if (_message != null)
        ...
        {
            var receiver = GetComponentInParent<MessageReceiver>();
        ...
        {
            if (target) return;
            var _receiver = target.GetComponent<MessageReceiver>();
            if (!_receiver) return;
            Message _message = messages.Count > 0 && messageIndex < messages.Count ? messages[messageIndex] : null;
        ...
        /// <summary>
        /// Send messages defined as <seealso cref="Message.sendByTrigger"/>.
        ...
        {
            if (!target) return;
            var _receiver = target.gameObject.GetComponent<MessageReceiver>();
            if (!_receiver) return;
            for (int i = 0; i < messages.Count; i++)
        ...
        public virtual void SendGlobal(string messageName)
        {
            ...
            for (int i = 0; i < _messages.Count; i++)
            {
                MessageReceiver.SendGlobal(_messages[i].name, _messages[i].message);
            }
        }
        ...
        {
            GlobalMessage _message = globalMessages.Count > 0 && messageIndex < globalMessages.Count ? globalMessages[messageIndex] : null;
            if (_message != null)
            {
                MessageReceiver.SendGlobal(_message.name, _message.message);
            }
        }
    }
}