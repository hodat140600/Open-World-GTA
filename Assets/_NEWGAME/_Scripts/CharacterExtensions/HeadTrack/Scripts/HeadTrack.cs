...
{
    [ClassHeader("HEAD TRACK", iconName = "ThirdPersonExtensions/headTrackIcon")]
    public class HeadTrack : ExtendMonoBehaviour
    {
        #region variables

        [EditorToolbar("Settings")]

        [HelpBox("If your character is not looking up/down, try changing the axis", HelpBoxAttribute.MessageType.Info)]
        ...
        [_Scripts.LeoHideInInspector("followCamera")]
        ...
        [Header("Limits")]
        [MinMax(minLimit = -180f, maxLimit = 180f)] public Vector2 horizontalAngleLimit = new Vector2(-100, 100);
        [MinMax(minLimit = -90f, maxLimit = 90f)] public Vector2 verticalAngleLimit = new Vector2(-80, 80);

        [HelpBox("Animations with vAnimatorTag Behavior will ignore the HeadTrack while is being played")]
        ...
        internal LookTarget currentLookTarget;
        internal LookTarget lastLookTarget;

        internal Quaternion currentLookRotation;
        internal List<LookTarget> targetsInArea = new List<LookTarget>();
        ...
        public virtual void SetLookTarget(LookTarget target, bool priority = false)
        {
        ...
        public virtual void RemoveLookTarget(LookTarget target)
        {
            if (targetsInArea.Contains(target))
            {
        ...
        public virtual void OnLost(Collider other)
        {
            ...
            {
                var _currentLookTarget = other.GetComponentInParent<LookTarget>();
