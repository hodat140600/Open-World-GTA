...
namespace _GAME._Scripts._ItemManager
{
    ...
    [Serializable]
    public partial class ItemDrawer
    {
        public Item item;
        ...
        protected ItemEditor defaultEditor;
        public List<ToolBars> itemToolBars;
        public delegate void OnDrawItem(ref List<Item> items, bool showObject = true, bool editName = false);
        ...
        {
            this.item = item;
            defaultEditor = (ItemEditor)Editor.CreateEditor(this.item, typeof(ItemEditor));
            FindDrawers();
        }

        void FindDrawers()
        {
            var methods = GetType().GetMethods().Where(m => m.GetCustomAttributes(typeof(ItemDrawerToolBarAttribute), true).Length > 0).ToArray();
            itemToolBars = new List<ToolBars>();
            itemToolBars.Add(new ToolBars("Properties", new OnDrawItem(DrawAllProperties)));
            for (int i = 0; i < methods.Length; i++)
            {
                string title = (methods[i].GetCustomAttributes(typeof(ItemDrawerToolBarAttribute), true)[0] as ItemDrawerToolBarAttribute).title;
        ...
        public virtual void DrawItem(ref List<Item> items, bool showObject = true, bool editName = false)
        {
            if (!item) return;
        ...
        public virtual void DrawAllProperties(ref List<Item> items, bool showObject, bool editName)
        {
        ...
        public virtual void DrawItemHeader(ref List<Item> items, bool showObject, bool editName)
        {
            if (showObject)
