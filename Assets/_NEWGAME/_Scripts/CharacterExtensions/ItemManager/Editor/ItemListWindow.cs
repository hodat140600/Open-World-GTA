...
namespace _GAME._Scripts._ItemManager
{
    public class ItemListWindow : EditorWindow
    {
        ...
        public Item addItem;
        public ItemDrawer addItemDrawer;
        public ItemDrawer currentItemDrawer;
        ...
        {
            ItemListWindow window = (ItemListWindow)GetWindow(typeof(ItemListWindow), false, "ItemList Editor");
            Instance = window;
        ...
        protected static void LoadSkin(ItemListWindow window)
        {
        ...
        public static void CreateWindow(ItemListData itemList, System.Action<int> OnSelectItem)
        {
            ItemListWindow window = CreateInstance<ItemListWindow>();
            //  Instance = window;
            window.itemList = itemList;
            LoadSkin(window);
            ...
            if (OnSelectItem == null)
            {
                ...
                if (inDragItens)
                {
                    ...
                    for (int i = 0; i < newItems.Count; i++)
                    {
                        ...
                        {
                            GUI.color = Color.red;
                            GUILayout.Label("EXIST"); EditorGUILayout.ObjectField(newItems[i], typeof(Item), false);
                        }
                        else
                        {
                            GUI.color = Color.white;
                            EditorGUILayout.ObjectField(newItems[i], typeof(Item), false);
                        }
                        GUI.color = Color.white;
                    ...
                    GUILayout.BeginVertical("box");
                    {
                        ...
                        GUILayout.BeginHorizontal();
                        {
                            ...
                            if (OnSelectItem == null)
                            {
                                ...
                                var duplicateImage = Resources.Load("ThirdPersonExtensions/duplicate") as Texture;
        ...
        public virtual void DrawFilter()
        {
            ...
            GUILayout.EndVertical();
            GUILayout.BeginHorizontal();
            search = GUILayout.TextField(search, GUILayout.Width(position.width - 50));
            GUILayout.Label(EditorGUIUtility.IconContent("Search Icon"), GUILayout.Height(20));
            GUILayout.EndHorizontal();
        }

        public virtual bool FilterItems(Item item)
        {
        ...
        {
            currentItemDrawer = currentItemDrawer != null ? currentItemDrawer.item == itemList.items[itemListIndex] ? null : new ItemDrawer(itemList.items[itemListIndex]) : new ItemDrawer(itemList.items[itemListIndex]);
        }
        ...
        public static void SetCurrentSelectedItem(int index)
        {
            ...
            {
                Instance.currentItemDrawer = Instance.currentItemDrawer != null ? Instance.currentItemDrawer.item == Instance.itemList.items[index] ? null : new ItemDrawer(Instance.itemList.items[index]) : new ItemDrawer(Instance.itemList.items[index]);
        ...
        protected virtual void DrawAddItem()
        {
            ...
            if (addItem != null)
            {
                ...
                if (addItemDrawer == null || addItemDrawer.item == null || addItemDrawer.item != addItem)
                    addItemDrawer = new ItemDrawer(addItem);
                bool isValid = true;
        ...
        private void AddItem(List<Item> items)
        {
            ...
            List<Item> itemsWithSameID = new List<Item>();
            for (int i = 0; i < items.Count; i++)
            {
                for (int z = 0; z < i; z++)
                {
        ...
        protected virtual void DuplicateItem(Item targetItem)
        {
        ...
        protected virtual int GetUniqueID(List<Item> items, int value = 0)
        {
            var result = value;
