...
namespace _GAME._Scripts._ItemManager
{
    ...
    public class ItemManagerEditor : EditorBase
    {
        ...
        {
            base.OnEnable();
            m_Logo = (Texture2D)Resources.Load("ThirdPersonExtensions/itemManagerIcon", typeof(Texture2D));
            manager = (ItemManager)target;
            itemReferenceList = serializedObject.FindProperty("startItems");
            skin = Resources.Load("dSkin") as GUISkin;

            ItemManagerUtilities.CreateDefaultEquipPoints(manager);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (openCloseWindow)
            {
                ...
                GUI.skin = skin;
                GUI.enabled = Application.isPlaying && ((ItemManager)target).GetItems().Count > 0;
                GUILayout.BeginHorizontal("box");
                if (GUILayout.Button("Drop All Items"))
                {
                    ((ItemManager)target).DropAllItens();
                }
                if (GUILayout.Button("Destroy All Items"))
                {
                    ((ItemManager)target).DestroyAllItems();
                }
        ...
        protected virtual void ShowItemListWindow(Item item = null)
        {
            if (item == null)
            {
                    ...
                    try
                    {
                        ...
                        if (points.isExpanded)
                        {
                            ...
                            if (inEdition[i])
                            {
                                ...
                                if (Array.Exists(array, point => point.gameObject.name.Equals(newPointNames[i])))
                                {
                                    valid = false;
                        ...
                        for (int a = 0; a < _array.Length; a++)
                        {
                            if (_array[a] != null)
                            {
                                ...
                                if (Array.FindAll(_array, point => point != null && point.gameObject.name.Equals(name)).Length > 1)
                                {
                                    _valid = false;
                                }
                            }
