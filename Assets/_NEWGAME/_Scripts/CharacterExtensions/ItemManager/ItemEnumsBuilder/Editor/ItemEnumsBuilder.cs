...
namespace _GAME._Scripts._ItemManager
{
    ...
    {
        public static bool NameEquals(this ItemEnumsBuilder.ItemEnumObject a, ItemEnumsBuilder.ItemEnumObject b)
        {
            return a.name.Equals(b.name);
        }
        public static bool FormatEquals(this ItemEnumsBuilder.ItemEnumObject a, ItemEnumsBuilder.ItemEnumObject b)
        {
    ...
    public class ItemEnumsBuilder
    {
        ...
        {
            string name = "ItemEnums";
            string copyPath = "Assets/_NEWGAME/_Scripts/_SHAME/CharacterExtensions/InventoryItems/ItemEnumsBuilder/" + name + ".cs";
            ...
            try
            {
                _itemTypeNames = Enum.GetNames(typeof(ItemType)).dToList();
            ...
            try
            {
                _itemAttributeNames = Enum.GetNames(typeof(ItemAttributes)).dToList();
            }
            catch
            {
            ...
            if (datas != null)
            {
                ...
                for (int i = 0; i < _itemTypeNames.Count; i++)
                {
                    ItemEnumsList data = datas.dToList().Find(d => d.itemTypeEnumValues.Contains(_itemTypeNames[i]));
                    if (data != null)
                    {
                ...
                for (int i = 0; i < _itemAttributeNames.Count; i++)
                {
                    ItemEnumsList data = datas.dToList().Find(d => d.itemAttributesEnumValues.Contains(_itemAttributeNames[i]));
                    if (data != null)
                    {
                                                     ...
                                                     List<string> itemTypesFormat = null, List<string> itemAttributesFormat = null)
        {
            ...
            using (StreamWriter outfile = new StreamWriter(copyPath))
            {
                ...
                outfile.WriteLine("namespace _GAME._Scripts._ItemManager {");
                outfile.WriteLine("     public enum ItemType {");
                if (itemTypes != null)
                {
                ...
                outfile.WriteLine("     }");
                outfile.WriteLine("     public enum ItemAttributes {");
