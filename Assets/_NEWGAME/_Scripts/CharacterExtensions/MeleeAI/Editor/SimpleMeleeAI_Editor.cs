...
namespace _GAME._Scripts._CharacterController._AI
{
    ...
    [CustomEditor(typeof(SimpleMeleeAI_Motor), true)]
    public class SimpleMeleeAI_Editor : EditorBase
    {
        protected override void OnEnable()
        {
            base.OnEnable();
            SimpleMeleeAI_Motor motor = (SimpleMeleeAI_Motor)target;
        ...
        void CreateSensor(SimpleMeleeAI_Motor motor)
        {
            if (Selection.activeTransform == null || !Selection.activeGameObject.activeSelf)
            {
                return;
            ...
            motor.sphereSensor = motor.GetComponentInChildren<SimpleMeleeAI_SphereSensor>();
            ...
            sensor.layer = layer;
            sensor.tag = "Weapon";
            motor.sphereSensor = sensor.AddComponent<SimpleMeleeAI_SphereSensor>();
        ...
        private static void CustomDrawGizmos(Transform aTarget, GizmoType aGizmoType)
        {
            ...
            if (Application.isPlaying)
            {
                SimpleMeleeAI_Motor motor = aTarget.GetComponent<SimpleMeleeAI_Motor>();
    ...
    public class PopUpLayerInfoEditor : EditorWindow
    {
        ...
        void OnGUI()
        {
            titleContent = new GUIContent("Warning!");
            minSize = rect;
