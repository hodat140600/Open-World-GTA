...
{
    using _ItemManager;
    [CustomEditor(typeof(SimpleMeleeAI_WeaponsControl))]
    public class SimpleMeleeAI_WeaponsControl_Editor : Editor
    {
        GUISkin skin;
        SimpleMeleeAI_WeaponsControl weaponCtrl;
        ...
        void OnEnable()
        {
            weaponCtrl = (SimpleMeleeAI_WeaponsControl)target;
            weaponCtrl.itemCollection = weaponCtrl.GetComponentInChildren<ItemCollection>(true);
        ...
        string[] GetItems()
        {
            ...
            {
                var items = weaponCtrl.itemCollection.items.FindAll(_item => weaponCtrl.itemCollection.itemListData.items.Find(_item2 => _item2.id == _item.id && _item2.type != ItemType.Consumable));
        ...
        public override void OnInspectorGUI()
        {
            ...
            weaponCtrl.itemCollection = (ItemCollection)EditorGUILayout.ObjectField("Item Collection", weaponCtrl.itemCollection, typeof(ItemCollection), true);
            if (weaponCtrl.itemCollection)
            {
                ...
                if (weaponCtrl.useLeftWeapon)
                {
                    ...
                    if (!weaponCtrl.randomLeftWeapon)
                    {
                        if (names.Length > 0)
                        {
                            if (!Array.Exists(ids, num => num == weaponCtrl.leftWeaponID))
                            {
                ...
                if (weaponCtrl.useRightWeapon)
                {
                    ...
                    if (!weaponCtrl.randomRightWeapon)
                    {
                        if (names.Length > 0)
                        {
                            if (!Array.Exists(ids, num => num == weaponCtrl.rightWeaponID))
                            {
