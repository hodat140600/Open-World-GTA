...
{
    [ClassHeader("Simple Melee AI", "")]
    public class SimpleMeleeAI_Controller : SimpleMeleeAI_Animator, IMeleeFighter
    {
        ...
        public const int   MIN_FIND_TARGET_ITERATION = 0;
        public const int   MAX_FIND_TARGET_ITERATION = 7;
        ...
        public virtual void SetCurrentTarget(Transform target)
        {
            if (target != currentTarget.transform)
            {
                ...
                currentTarget.character = target.GetComponent<IHealthController>();
            }
        ...
        protected IEnumerator FindTarget()
        {
            ...
            WaitForSeconds[] cachedWaitForSeconds = new WaitForSecondsCached().GenerateWaitForSeconds(MIN_FIND_TARGET_ITERATION, MAX_FIND_TARGET_ITERATION);
            int index;
            
            while (true)
            {
                index = Random.Range(0, cachedWaitForSeconds.Length);
                yield return cachedWaitForSeconds[index];
                if (CurrentHealth > 0)
                {
                    SetTarget();
                    CheckTarget();
        ...
        protected virtual IEnumerator Wander()
        {
            ...
            {
                yield return null;
                destination = transform.position + Quaternion.AngleAxis(Random.Range(-120, 120), transform.up) * transform.forward * (patrollingStopDistance + 4);
...
#if USE_WAYPOINT_INGAME
        protected virtual int GetCurrentWaypoint()
        {
            return 0;
        }
#endif
...
#region AI Melee Combat        
        ...
        IEnumerator ResetAttackCount()
        {
            ...
            {
                value = Random.Range(minTimeToAttack, maxTimeToAttack);
            }
