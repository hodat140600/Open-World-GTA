...
{
    public class SimpleMeleeAI_SphereSensor : MonoBehaviour
    {
        ...
        public virtual IHealthController GetTargetvCharacter()
        {
            if (targetsInArea.Count > 0)
            {
                SortCharacters();
                if (targetsInArea.Count > 0)
                {
                    var vChar = targetsInArea[0].GetComponent<IHealthController>();
        ...
        protected virtual void SortCharacters()
        {
            for (int i = targetsInArea.Count - 1; i >= 0; i--)
            {
                ...
                if (t == null || dist > lastDetectionDistance || t.GetComponent<IHealthController>() == null)
                {
        ...
        public virtual void CheckTargetsAround(float FOV, float minDistance, float maxDistance, TagMask detectTags, LayerMask detectMask, bool getTargetFromDistance = false)
        {
            ...
            targetsInArea = System.Array.ConvertAll(targetsAround, c => c.transform).dToList();
        }
