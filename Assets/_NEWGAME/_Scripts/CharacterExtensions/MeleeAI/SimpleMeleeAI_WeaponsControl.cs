...
namespace _GAME._Scripts._CharacterController._AI
{
    using _ItemManager;
    using _Melee;
    public class SimpleMeleeAI_WeaponsControl : MonoBehaviour
    {
        #region Variables
        ...
        protected SimpleMeleeAI_Controller ai;
        protected MeleeManager manager;
        protected Item leftWeaponItem, rightWeaponItem;
        protected GameObject leftWeapon, rightWeapon;
        protected List<Item> weaponItems = new List<Item>();
    ...
    protected System.Random random;
        ...
        IEnumerator Start()
        {
            ...
            ai = GetComponent<SimpleMeleeAI_Controller>();
            manager = GetComponent<MeleeManager>();
            ...
            if (itemCollection && ai && manager)
            {
                ...
                for (int i = 0; i < itemCollection.items.Count; i++)
                {
                    ...
                    {
                        var item = itemCollection.itemListData.items.Find(_item => _item.id == itemCollection.items[i].id && _item.type == ItemType.MeleeWeapon);
                        if (item != null)
                        {
        ...
        void GetItemWeapon(int id, ref Item weaponItem, MeleeType type)
        {
            if (weaponItems.Count > 0)
            {
                weaponItem = weaponItems.Find(_item => _item.id == id &&
                            _item.originalObject != null && _item.originalObject.GetComponent<MeleeWeapon>() != null &&
                            (_item.originalObject.GetComponent<MeleeWeapon>().meleeType == MeleeType.AttackAndDefense || _item.originalObject.GetComponent<MeleeWeapon>().meleeType == type));

                weaponItems.Remove(weaponItem);
            }
        }

        void GetRandomWeapon(ref Item weaponItem, MeleeType type)
        {
            if (weaponItems.Count > 0)
            {
        ...
        IEnumerator EquipItemRoutine(bool flipEquip, Item item, GameObject weapon)
        {
            equipCalls++;
            ...
            if (!inUnequip)
            {
                ...
                if (weapon != null && item != null)
                {
                    ...
                    var type = item.type;
                    if (type != ItemType.Consumable && !ai.isDead)
                    {
        ...
        IEnumerator UnequipItemRoutine(bool flipEquip, Item item, GameObject weapon)
        {
