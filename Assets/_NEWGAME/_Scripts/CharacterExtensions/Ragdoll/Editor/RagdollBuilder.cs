...
namespace _GAME._Scripts._CharacterController
{
    class RagdollBuilder : ScriptableWizard
    {
        ...
        public RagdollGenericTemplate genericTemplate = null;
        [Header("--- Bones ---")]
        ...
        [MenuItem("DatHQ/Basic Locomotion/Components/Ragdoll")]
        static void CreateWizard()
        {
            DisplayWizard("Create Ragdoll", typeof(RagdollBuilder));
        ...
        void PrepareBones()
        {
            ...
            if (animator != null && avatarType == RagdollAvatarType.Human)
            {
                GetHumanBones();
            }
            else
        ...
        void OnWizardCreate()
        {
            ...
            var hasRagdoll = Selection.activeGameObject.GetComponent<Ragdoll>();
            if (!hasRagdoll)
            {
        ...
        void BuildBodies()
        {
            foreach (BoneInfo bone in bones)
            {
                ...
                var hasDamageReceiver = bone.anchor.gameObject.GetComponent<DamageReceiver>();
                if (!hasDamageReceiver)
                {
                    bone.anchor.gameObject.AddComponent<DamageReceiver>();
                }
