...
namespace _GAME._Scripts._CharacterController._AI
{
    ...
    public class WaypointAreaEditor : Editor
    {
        ...
        [MenuItem("GameObject/DatHQ/FSM AI/New Waypoint Area", false, -100)]
        [MenuItem("DatHQ/FSM AI/Components/New Waypoint Area")]
        static void NewWaypointArea()
        {
            var wp = new GameObject("WaypointArea", typeof(WaypointArea));
...
#endif
        [MenuItem("DatHQ/Melee Combat/Components/New Waypoint Area")]
        ...
        void OnEnable()
        {
            m_Logo = (Texture2D)Resources.Load("icon_d2", typeof(Texture2D));
            tool = Tools.current;
            pathArea = (WaypointArea)target;
            editMode = serializedObject.FindProperty("editMode");
            if (pathArea.waypoints == null)
                pathArea.waypoints = new List<Waypoint>();
        ...
        void OnSceneGUI()
        {
            ...
            if (pathArea.waypoints.Count > 0)
            {
                ...
                foreach (Waypoint node in pathArea.waypoints)
                {
                    if (node != null)
                    {
                        ...
                        if (editMode.boolValue)
                            Handles.color = new Color(0, 0, 1, .2f * 8);
                        ...
                        if (currentNode == null || !currentNode.Equals(node))
                        {
                            ...
                            if (node.subPoints != null && node.subPoints.Count > 0)
                            {
                                ...
                                foreach (Point patrolPoint in node.subPoints)
                                {
                                    if (patrolPoint != null)
                                    {
            ...
            if (currentNode && editMode.boolValue)
            {
                ...
                if (currentNode.subPoints == null) currentNode.subPoints = new List<Point>();
                if (currentNode.subPoints.Count > 0)
                {
                    ...
                    foreach (Point patrolPoint in currentNode.subPoints)
                    {
                        Handles.color = Color.cyan;
                        Handles.DrawLine(patrolPoint0.transform.position, patrolPoint.position);
                        patrolPoint0 = patrolPoint;
        ...
        public override void OnInspectorGUI()
        {
            ...
            try
            {
                ...
                {
                    var vPoint = new SerializedObject(waypoints.GetArrayElementAtIndex(indexOfWaypoint).objectReferenceValue as Waypoint);
        ...
        public void SetVisiblePoints(bool value)
        {
            ...
            if (pathArea.waypoints != null && pathArea.waypoints.Count > 0)
            {
                foreach (Waypoint wP in pathArea.waypoints)
                {
                    if (wP != null)
                    {
                        if (value == true)
        ...
        void DrawWaypoint(SerializedObject waypoint, SerializedProperty waypoints)
        {
            if (waypoint != null)
            {
                ...
                EditorGUILayout.Space();
                EditorGUILayout.PropertyField(waypoint.FindProperty("Id"));
        ...
        void DrawVPoint(SerializedProperty vPoints)
        {
            if (vPoints != null && vPoints.arraySize > 0)
            {
                ...
                for (int i = 0; i < vPoints.arraySize; i++)
                {
                    ...
                    if (!PointIsInNavMesh((vPoints.GetArrayElementAtIndex(i).objectReferenceValue as Point).transform.position))
                    {
                        GUI.color = Color.white;
                        EditorGUILayout.HelpBox("Out of NavMesh", MessageType.Error);
                        Repaint();
        ...
        {
            var obj = (waypoints.GetArrayElementAtIndex(index).objectReferenceValue as Point).gameObject;
            waypoints.DeleteArrayElementAtIndex(index);

            DestroyImmediate(obj);
        }

        void CreateNode(WaypointArea wayArea, Vector3 position)
        {
            var nodeObj = new GameObject("node");
            var node = nodeObj.AddComponent<Waypoint>();
            node.subPoints = new List<Point>();
        ...
        void CreatePatrolPoint(WaypointArea wayArea, Vector3 position)
        {
            if (currentNode)
            {
                if (currentNode.subPoints == null) currentNode.subPoints = new List<Point>();
                var nodeObj = new GameObject("patrolPoint");
                var node = nodeObj.AddComponent<Point>();
                nodeObj.transform.position = position;
        ...
        {
            var wP = ((WaypointArea)target).transform.GetComponentsInChildren<Waypoint>();

            if (waypoints.Count != wP.Length)
                waypoints = wP.ToList();
            foreach (Waypoint waypoint in waypoints)
            {
                var vP = waypoint.transform.GetComponentsInChildren<Point>();
