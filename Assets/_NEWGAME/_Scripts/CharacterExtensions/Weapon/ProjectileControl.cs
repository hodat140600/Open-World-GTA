...
{
    [ClassHeader("Projectile Control", "The damage value is changed from minDamage, maxDamage, DropOffStart, DropOffEnd of the ShooterWeapon", openClose = false)]
    public class ProjectileControl : ExtendMonoBehaviour
    {
        public BulletLifeSettings bulletLifeSettings;
        ...
        public ProjectileInstantiateData instantiateData;
        ...
        {
            transform.SetParent(ObjectContainer.root, true);
            ...
            if (Physics.Linecast(previousPosition, transform.position + transform.forward * 0.5f, out hitInfo, hitLayer))
            {
                ...
                if (!(ignoreTags.Contains(hitInfo.collider.gameObject.tag) || shooterTransform != null && hitInfo.collider.transform.IsChildOf(shooterTransform)))
                {
                    ...
                    if (damage.damageValue > 0)
                    {
                        ...
                        hitInfo.collider.gameObject.ApplyDamage(damage, damage.sender ? damage.sender.GetComponent<IMeleeFighter>() : null);
                    }
                        ...
                        if (bulletLifeSettings)
                        {
                            ...
                            if (!crossed && !bulletLifeInfo.ricochet)
                            {
                                ...
                                if (trail && trail.gameObject != this.gameObject)
                                {
                                    ...
                                    trail.transform.SetParent(ObjectContainer.root);
                                }
                                Destroy(gameObject);
                                return;
                            }
        ...
        {
            other.SetParent(ObjectContainer.root, true);
        }
