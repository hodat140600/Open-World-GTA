...
namespace _GAME._Scripts._CharacterController._Shooter
{
    ...
    public class ShooterWeaponBase : ExtendMonoBehaviour
    {
        #region Variables
        ...
        [SerializeField, _Scripts.LeoHideInInspector("isInfinityAmmo", true)]
        public int ammo;

        [EditorToolbar("Layer & Tag")]
        ...
        [System.Serializable]
        public class OnInstantiateProjectile : UnityEvent<ProjectileControl> { }
        ...
        public virtual void Shoot(Vector3 aimPosition, Transform _sender = null, UnityAction<bool> successfulShot = null)
        {
            if (HasAmmo())
            {
                ...
                UseAmmo();
                sender = _sender != null ? _sender : transform;
        ...
        protected virtual void ShootBullet(Vector3 aimPosition)
        {
            ...
            if (dispersion > 0 && projectile)
            {
                for (int i = 0; i < projectilesPerShot; i++)
                {
                    ...
                    var pCtrl = bulletObject.GetComponent<ProjectileControl>();
        ...
        protected virtual ProjectileControl CreateProjectileData(Vector3 aimPosition, float velocityChanged, Vector3 dispersionDir, ProjectileControl pCtrl)
        {
            pCtrl.instantiateData = new ProjectileInstantiateData
            {
                aimPos = aimPosition,
                dir = dispersionDir,
                vel = velocityChanged
